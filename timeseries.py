...
def forecast_deviation(feature=None, forecast_provider="accuweathercom", city="Berlin"):
	...
	matplotlib.rcParams.update({'font.size': 8})
	plt.figure()
	plt.title("Comparison of MSE by weekday")
	# concat.plot()
	weekday_deviation_1.plot(legend=True)
	weekday_deviation_2.plot(legend=True)
	weekday_deviation_3.plot(legend=True)
	weekday_deviation_4.plot(legend=True)
